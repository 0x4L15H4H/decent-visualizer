...
- **`/backend`** — the backend's runtime secrets. `supabase_url` and `supabase_service_key` are written by Terraform on apply. `gemini_api_key` must be added by hand (used for bean photo extraction). The backend pulls all of these at startup. The Supabase database password is generated by Terraform and stays in its state.
